...
#include "secrets/soporte_pies_secrets.h"
#include "utils/iot_utils.hpp"
#include "utils/multiple_rfid_utils.hpp"
#define SHADOW_GET_TOPIC "$aws/things/soporte_pies/shadow/get"
#define SHADOW_GET_ACCEPTED_TOPIC "$aws/things/soporte_pies/shadow/get/accepted"
#define SHADOW_UPDATE_TOPIC "$aws/things/soporte_pies/shadow/update"
#define SHADOW_UPDATE_ACCEPTED_TOPIC "$aws/things/soporte_pies/shadow/update/accepted"
#define SHADOW_UPDATE_DELTA_TOPIC "$aws/things/soporte_pies/shadow/update/delta"
#define RESET_TOPIC "soporte_pies/reset"
...
void loop()
{
	now = time(nullptr);
	if (!client.connected()) {
		reconnect(THINGNAME, "soporte_pies/status");
	}
	client.loop();
	printMultipleRFID();
	if (newRFIDAppeared) {
